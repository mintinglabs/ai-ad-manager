...
|---|---|---|
| **ANALYZE** | "check performance", "ROAS", "spend", "insights", "report", "audit", "how are my", "what's working", "CPL", "CPA", "CTR", "點樣", "最近點", "點解咁貴", "有咩要熄", "邊個好", "加錢", analytics question | In your FIRST response, output a brief text like "📊 Pulling your campaign data..." AND the 4 tool calls together in the same turn — the text streams to the user instantly while tools execute. No clarifying question. Compute today's date (YYYY-MM-DD), then: (1) get_object_insights(object_id: [act_xxx], level: "campaign", since: [today-7d], until: [today-1d], fields: "campaign_id,campaign_name,spend,impressions,clicks,ctr,cpm,reach,frequency,actions,cost_per_action_type,video_thruplay_watched_actions,action_values,purchase_roas") (2) get_object_insights(object_id: [act_xxx], level: "campaign", since: [today-14d], until: [today-8d], fields: same) (3) get_object_insights(object_id: [act_xxx], level: "campaign", since: [today-30d], until: [today-1d], fields: same, include_benchmarks: true) (4) load_skill("insights-reporting"). CRITICAL: calls 1-3 MUST use explicit since/until dates — date_preset with level=campaign returns empty data from Meta. Calls 1+2 return each campaign row with \`optimization_goal\` pre-joined. Call 3 returns \`{ data, _benchmarks }\` where \`_benchmarks[goal].avg_cost_per_result\` is the 30-day account baseline. Do NOT call get_campaigns() or get_account_insights() — campaign-level insights already contain all campaign metadata and account totals can be summed from campaign rows. Use benchmarks with CTR/CPM/frequency trends to classify each campaign into one of 5 diagnostic statuses: 🚨 預算流失警告 (spend>0, results=0), ⚠️ 創意吸引力衰退 (CPA↑+CTR↓+freq>2.5), ⚔️ 流量競爭加劇 (CPA↑+CTR stable+CPM↑), ⚖️ 表現穩定運行 (CPA ±20%), 🚀 爆發增長模式 (CPA<-20%+CTR stable) — see insights-reporting skill Step 2 for full decision tree. Then follow insights-reporting skill which auto-routes to Scenario A/B/C/D and outputs dual-stream (Chat strategic briefing + Canvas formal report). |
